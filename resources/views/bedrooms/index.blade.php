        ...
        <div class="row" >
                ...
                <div class="col-md-4">
                    <div class="card mb-3 shadow-sm" >
                        <h3 style="text-align:center;">{{$bedroom->name}}</h3>
                        
                        <a href="/bedrooms/{{$bedroom->id}}">
                        <img src="/storage/photos/bedrooms/{{$bedroom->image}}" alt="leather seat" class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img"  rect fill="#55595c" width="100%" height="100%"/>
                        </a>
                          <div class="card-body">
                                <p class="card-text">{{$bedroom->description}}</p>
                                <p class="card-text" >KES: {{$bedroom->price}}</p>
                              
                                <div class="d-flex justify-content-between align-items-center">
                                      <div class="btn-group">
                                          <a href="/bedrooms/{{$bedroom->id}}"><button type="button" class="btn btn-sm btn-outline-secondary">View</button></a>

                                          @if(!Auth::guest())
                                          @if(Auth::user()->id == $bedrooms->user_id)
                                          <a href="/bedrooms/{{$bedroom->id}}/edit"><button type="button" class="btn btn-sm btn-outline-secondary">Edit</button></a>
                                          @endif
                                          @endif
                                      
                                      </div>
                                    
                                    <small class="text-muted">{{$bedroom->created_at->diffForHumans()}}</small>
                                    <small class="text-muted">{{$bedroom->condition}}</small>
                                </div>
                          </div>
