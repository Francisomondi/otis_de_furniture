...
<div class="container">
        <div class="row featurette">
                <div class="col-md-7">
                  <h2 class="featurette-heading">{{$bedrooms->name}} <span class="text-muted">KES {{$bedrooms->price}}</span></h2>
                  <p class="lead"> {{$bedrooms->description}}.</p>
                </div>
                <div class="col-md-5">
                        <img src="/storage/photos//bedrooms/{{$bedrooms->image}}" alt="leather seat" class="bd-placeholder-img bd-placeholder-img-lg featurette-image img-fluid mx-auto" width="500" height="500" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" aria-label="Placeholder: 500x500">
                    </div>
                    </div>
                    ...
                    @if(Auth::user()->id == $bedrooms->user_id)
                       <div class="row">
                               <a href="/bedrooms" class="btn btn-sm btn-outline-primary">Back</a>
                               
                         {!!Form::open(['action'=> ['bedroomsController@destroy',$bedrooms->id],'method'=> 'POST','class'=>'pull-right'])!!}
