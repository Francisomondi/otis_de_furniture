...
<main role="main">
    ...
    <div class="album py-5 bg-light">
        ...
        <div class="row" >
                @if(count($carbinets)>0)

                @foreach($carbinets as $carbinet)
            
                <div class="col-md-4">
                    <div class="card mb-3 shadow-sm" >
                        <h3 style="text-align:center;">{{$carbinet->name}}</h3>
                        
                        <a href="/carbinets/{{$carbinet->id}}">
                        <img src="/storage/photos/carbinet/{{$carbinet->image}}" alt="leather seat" class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img"  rect fill="#55595c" width="100%" height="100%"/>
                        </a>
                          <div class="card-body">
                                <p class="card-text">{{$carbinet->description}}</p>
                                <p class="card-text" >KES: {{$carbinet->price}}</p>
                              
                                <div class="d-flex justify-content-between align-items-center">
                                      <div class="btn-group">
                                          <a href="/carbinets/{{$carbinet->id}}"><button type="button" class="btn btn-sm btn-outline-secondary">View</button></a>
                                         
                                          <a href="/carbinets/{{$carbinet->id}}/edit"><button type="button" class="btn btn-sm btn-outline-secondary">Edit</button></a>
                                         
                                      
                                      </div>
                                    
                                    <small class="text-muted">{{$carbinet->created_at->diffForHumans()}}</small>
                                    <small class="text-muted">{{$carbinet->condition}}</small>
                                </div>
                          </div>
