...
<main role="main">
    ...
    <div class="album py-5 bg-light">
        ...
        <div class="row" >
                @if(count($barstools)>0)

                @foreach($barstools as $barstool)
            
                <div class="col-md-4">
                    <div class="card mb-3 shadow-sm" >
                        <h3 style="text-align:center;">{{$barstool->name}}</h3>
                        
                        <a href="/barstools/{{$barstool->id}}">
                        <img src="/storage/photos/{{$barstool->image}}" alt="leather seat" class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img"  rect fill="#55595c" width="100%" height="100%"/>
                        </a>
                          <div class="card-body">
                                <p class="card-text">{{$barstool->description}}</p>
                                <p class="card-text" >KES: {{$barstool->price}}</p>
                              
                                <div class="d-flex justify-content-between align-items-center">
                                      <div class="btn-group">
                                          <a href="/barstools/{{$barstool->id}}"><button type="button" class="btn btn-sm btn-outline-secondary">View</button></a>
                                          @if(!Auth::guest())
                                          @if(Auth::user()->id == $barstools->user_id)
                                          <a href="/barstools/{{$barstool->id}}/edit"><button type="button" class="btn btn-sm btn-outline-secondary">Edit</button></a>
                                          @endif
                                          @endif
                                      
                                      </div>
                                    
                                    <small class="text-muted">{{$barstool->created_at->diffForHumans()}}</small>
                                    <small class="text-muted">{{$barstool->condition}}</small>
                                </div>
                          </div>
...
<hr class="featurette-divider">-->
    ...
    <footer class="container">
      ...
      <p>&copy; Noble Furitures, Inc. &middot; <a href="#">Privacy</a> &middot; <a href="#">Terms</a></p>
    </footer>
  </main>
