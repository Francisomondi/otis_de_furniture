...
<main role="main">
    ...
    <div class="album py-5 bg-light">
        ...
        <div class="row" >
                @if(count($leathersofas)>0)

                @foreach($leathersofas as $leathersofa)
            
                <div class="col-md-4">
                    <div class="card mb-3 shadow-sm" >
                 <h3 style="text-align:center;">{{$leathersofa->name}}</h3>
                        
                        <a href="/leathersofas/{{$leathersofa->id}}">
                        <img src="/storage/photos/leathersofa/{{$leathersofa->image}}" alt="leather seat" class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img"  rect fill="#55595c" width="100%" height="100%"/>
                        </a>
                          <div class="card-body">
                                <p class="card-text">{{$leathersofa->description}}</p>
                                <p class="card-text" >KES: {{$leathersofa->price}}</p>
                              
                                <div class="d-flex justify-content-between align-items-center">
                                      <div class="btn-group">
                                          <a href="/leathersofas/{{$leathersofa->id}}"><button type="button" class="btn btn-sm btn-outline-secondary">View</button></a>
                                          <a href="/leathersofas/{{$leathersofa->id}}/edit"><button type="button" class="btn btn-sm btn-outline-secondary">Edit</button></a>
                                      
                                      </div>
                                    
                                    <small class="text-muted">{{$leathersofa->created_at->diffForHumans()}}</small>
                                    <small class="text-muted">{{$leathersofa->condition}}</small>
                                </div>
                          </div>
